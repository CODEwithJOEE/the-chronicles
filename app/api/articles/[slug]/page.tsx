...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  // SEO Metadata
  const pageTitle = article.meta_title || `${article.title} - The Chronicle`;
  const pageDescription =
    article.meta_description ||
    article.content.replace(/<[^>]*>/g, "").substring(0, 155) + "...";
  const pageKeywords =
    article.meta_keywords ||
    `news, chronicle, article, ${article.category_name || "blog"}`;
    ...
    <>
      <head>
        ...
        {article.featured_image && (
          <meta
            property="og:image"
            content={`/uploads/${article.featured_image}`}
          />
        )}
      ...
      <div className="container py-8">
        ...
        <div className="fixed top-0 left-0 w-full h-1 bg-transparent z-50">
          <div
            id="progressBar"
            className="h-full bg-[#ff9f1c] w-0 transition-width duration-100"
          />
        </div>
            ...
            <article>
              ...
              {article.featured_image && (
                <div className="relative h-64 md:h-96 mb-8 rounded-lg overflow-hidden">
                  <Image
                    src={`/uploads/${article.featured_image}`}
                    alt={article.title}
                    fill
